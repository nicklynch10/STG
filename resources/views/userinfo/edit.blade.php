...
<form method="post" action="{{url('/info/save')}}" style="width: 100%;">
{{ csrf_field() }}
  <div style="margin:  20px 50px; ">
    <div class="mdl-color-text--light-blue-300" style="font-size: 35px; text-align: center; line-height: 60px;"> {{Auth::user()->morphname()}}'s Info </div>
    <div style="width: 50%;">
    ...
    <textarea style="width:100%; min-height:60px;" class="mdl-textfield__input" name="field{{$key}}" id="field{{$key}}">{{Auth::user()->$fieldname}}</textarea>
    <label class="mdl-textfield__label" for="field{{$key}}">{{explode('?',$q)[0]}}?</label>
  </div>
  </div>
      </div>
    <?php } ?>
