...
{{ $temp->format('l')}}
		...
		@for($e=0;$e<96;$e++)
		<?php 
		$under_temp = Carbon\Carbon::parse($temp)->addHours($e/4)->addMinutes(($e%4)*15);
		$un_str = $t.'||new||'.$e;
		?>
			...
			<input class="input_under_seventh" type="hidden" name="{{$t.'||new||'.$e}}" value="0">
   			</div>
   			@else
   			<div index="{{$e}}" data-datetime='{{$under_temp}}'
			data-time="{{$under_temp->format('g:i A \\o\\n l\\, F j')}}"
			class="under_seventh event_click unavailable"> 
			<input class="input_under_seventh" type="hidden" name="{{$t.'||new||'.$e}}" value="1">
   			</div>
   			@endif
   		@endfor
    </div>
